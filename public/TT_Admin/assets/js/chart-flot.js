  ...
  CMPLTADMIN_SETTINGS.chartFlot = function () {
    ...
    if ($('#flot-realtime').length) {
      ...
      function RealTimegetRandomData() {
        if (rtdata.length > 0) rtdata = rtdata.slice(1);
    ...
    if ($('#flot-navigate').length) {
      ...
      function addArrow(dir, right, top, offset) {
        $(
          "<img class='button' src='assets/plugins/flot-chart/images/arrow-" +
            dir +
            ".gif' style='right:" +
            right +
            'px;top:' +
    ...
    if ($('#flot-toggle').length) {
      ...
      $.each(togdatasets, function (key, val) {
        choiceContainer.append(
          "<br/><input class='iCheck' type='checkbox' name='" +
            key +
    ...
    if ($('#flot-track').length) {
      ...
      function updateLegend() {
        ...
        var i,
          j,
          trackdataset = trackplot.getData();
    ...
    if ($('#democombine').length) {
      ...
      var combineplot = $.plot($('#flotcombine'), combinedata, combineoptions);
    }
    /*-----------------------------------------------------*/
  };
