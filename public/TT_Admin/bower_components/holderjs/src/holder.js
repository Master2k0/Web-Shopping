...
(function (register, global, undefined) {
  ...
  var Holder = {
    ...
    run: function (userOptions) {
      ...
      renderSettings.renderer = options.renderer
        ? options.renderer
        : App.setup.renderer;
      if (
        App.setup.renderers.join(',').indexOf(renderSettings.renderer) === -1
      ) {
        renderSettings.renderer = App.setup.supportsSVG
          ? 'svg'
      ...
      renderSettings.noFontFallback = options.noFontFallback
      ...
      for (i = 0; i < bgnodes.length; i++) {
        ...
        var dataBackgroundImage = bgnodes[i].getAttribute(
          'data-background-src',
        );
        var rawURL = null;
  ...
  function prepareDOMElement(mode, el, flags, _renderSettings) {
    ...
    if (mode == 'image' || mode == 'fluid') {
      setAttr(el, {
        alt: theme.text
          ? (theme.text.length > 16
              ? theme.text.substring(0, 16) + '…'
              : theme.text) +
            ' [' +
            dimensionsCaption +
            ']'
  ...
  function buildSceneGraph(scene) {
    scene.font = {
      ...
      size: textSize(
        scene.width,
        scene.height,
        scene.theme.size ? scene.theme.size : App.defaults.size,
      ),
      units: scene.theme.units ? scene.theme.units : App.defaults.units,
      weight: scene.theme.fontweight ? scene.theme.fontweight : 'bold',
    };
    scene.text = scene.theme.text
      ? scene.theme.text
    ...
    switch (scene.flags.textmode) {
      case 'literal':
        scene.text =
          scene.flags.dimensions.width + 'x' + scene.flags.dimensions.height;
        break;
      case 'exact':
  ...
  function updateResizableElements(element) {
    ...
    for (var i in images) {
      ...
      if (el.holderData) {
        ...
        var dimensions = dimensionCheck(
          el,
          Holder.invisibleErrorFn(updateResizableElements),
        );
        if (dimensions) {
  ...
  function setInitialDimensions(el) {
    if (el.holderData) {
      var dimensions = dimensionCheck(
        el,
        Holder.invisibleErrorFn(setInitialDimensions),
      );
      if (dimensions) {
        var flags = el.holderData.flags;
  ...
  var sgCanvasRenderer = (function () {
    ...
    return function (sceneGraph) {
      ...
      ctx.fillRect(
        0,
        0,
        App.dpr(root.children.holderBg.width),
        App.dpr(root.children.holderBg.height),
      );
      ...
      for (var lineKey in textGroup.children) {
        ...
        for (var wordKey in line.children) {
          ...
          var y = App.dpr(
            textGroup.y + line.y + word.y + textGroup.properties.leading / 2,
          );
  ...
  function serializeSVG(svg, renderSettings) {
    ...
    if (renderSettings.svgXMLStylesheet) {
      ...
      var xmlpi = xml.createProcessingInstruction(
        'xml',
