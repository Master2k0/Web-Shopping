...
$.fn.dataTableExt.oPagination.ellipses = {
  ...
  fnInit: function (oSettings, nPager, fnCallbackDraw) {
    ...
    oFirst.click(
      { fnCallbackDraw: fnCallbackDraw, oSettings: oSettings, sPage: 'first' },
      that.fnClickHandler,
    );
    oPrevious.click(
      {
    ...
    oNext.click(
      { fnCallbackDraw: fnCallbackDraw, oSettings: oSettings, sPage: 'next' },
      that.fnClickHandler,
    );
    oLast.click(
      { fnCallbackDraw: fnCallbackDraw, oSettings: oSettings, sPage: 'last' },
      that.fnClickHandler,
    );
  ...
  fnUpdate: function (oSettings, fnCallbackDraw) {
    ...
    if (
      oSettings._iTotalPages === 0 ||
      oSettings._iCurrentPage === oSettings._iTotalPages
    ) {
    ...
    if (1 < oSettings._iFirstPage) {
      oNumbers.prepend(
        '<span class="' + oClasses.sPageEllipsis + '">...</span>',
      );
    }
  ...
  fnUpdateState: function (oSettings) {
    var iCurrentPage = Math.ceil(
        (oSettings._iDisplayStart + 1) / oSettings._iDisplayLength,
      ),
