...
(function (window, document, $, undefined) {
  $.fn.dataTable.SelectedLengthMenu = function (oSettings) {
    ...
    var sSelectedMenu =
      '<select name="' + oSettings.sTableId + '_selectedFilter">';
    if (typeof oFilterSelectedOptions === 'object') {
      sSelectedMenu +=
        '<option value="All">' + oFilterSelectedOptions.AllText + '</option>';
      sSelectedMenu +=
        '<option value="Selected">' +
        oFilterSelectedOptions.SelectedText +
        '</option>';
    } else {
      sSelectedMenu +=
        '<option value="All">' + oFilterSelectedOptions[0] + '</option>';
      sSelectedMenu +=
        '<option value="Selected">' + oFilterSelectedOptions[1] + '</option>';
    }
    ...
    nLength.innerHTML =
      '<span>' +
      sLengthMenu
        .replace('_MENU_', sStdMenu)
        .replace('_SELECTEDMENU_', sSelectedMenu) +
      '</span>';

    var $lengthSelect = $(
      'select[name="' + oSettings.sTableId + '_length"]',
      nLength,
    );
     ...
     */
    $lengthSelect
      .find('option[value="' + oSettings._iDisplayLength + '"]', nLength)
      .attr('selected', true);
